...
class CitaController {
    ...
    static async marcarConfirmada(req, res) {
        try {
            const errors = validationResult(req);
            if (!errors.isEmpty()) {
                return res.status(400).json({
            ...
            await Cita.marcarConfirmada({ idCita, usuarioId });
            ...
            console.error('Error en marcarConfirmada:', error);
            return res.status(error.status || 500).json({
                success: false,
                message: error.message || 'Error interno del servidor',
                code: error.code || 'INTERNAL_ERROR'
    ...
    static async marcarNoAsistio(req, res) {
        try {
            const errors = validationResult(req);
            if (!errors.isEmpty()) {
                return res.status(400).json({
            ...
            await Cita.marcarNoAsistio({ idCita, usuarioId });
